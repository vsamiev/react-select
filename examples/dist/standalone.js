...
var Select = _react2['default'].createClass({
	...
	getDefaultProps: function getDefaultProps() {
		return {
			...
			valueKey: 'value',
			createNewOption: { id: 'createNew', label: '' },
			keepOpen: false
		};
	},
  ...
  */
	...
	selectValue: function selectValue(_value) {
		var _this3 = this;

		// after state update we are loosing input value
		var value = _extends({}, _value);
		...
		if (this.props.multi) {
			...
			this.setState({
				isOpen: this.props.keepOpen || false,
				inputValue: '',
	...
	filterOptions: function filterOptions(excludeOptions) {
		...
		if (options.length == 0 && this.props.allowCreate) {
			var createNewOption = this.newOptionObject(this.state.inputValue);
			options = [createNewOption].concat(_toConsumableArray(options));
		}
		return options;
	},

	newOptionObject: function newOptionObject(filterValue) {
		var option = this.props.createNewOption;
		option[this.props.labelKey] = this.props.addLabelText.replace('{label}', filterValue);
		return option;
	},
